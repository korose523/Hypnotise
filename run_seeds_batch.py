"""run_seeds_batch.py — Run a small batch of seeds for one target.

Usage: python run_seeds_batch.py <target> <seed1> <seed2> ...
Example: python run_seeds_batch.py SEED 1111 2222 3333 4444 5555
"""
...
def save_results(results):
    """Save results to main file with simple locking via temp file."""
...
def run_batch(target, seeds, out_path=None):
    ...
    print('Target=%s  n_src=%d  tgt=%d  groups=%d' % (target, len(ys), len(yt), n_grp))

    new_results = []
    for seed in seeds:
        if (target, seed) in done:
            print('  s=%d: SKIP (already done)' % seed)
            continue
        if n_grp < 2:
            print('  s=%d: SKIP (<2 groups)' % seed)
            continue
    ...
    # Save to target-specific file to avoid race conditions
    save_path = out_path or (RESULT_PATH.replace('.json', '_%s.json' % target))
    os.makedirs(os.path.dirname(save_path), exist_ok=True)
    with open(save_path, 'w') as f:
        json.dump(new_results, f, indent=1)
    print('Batch done. Saved %d new results to %s' % (len(new_results), save_path))


if __name__ == '__main__':
    if len(sys.argv) < 3:
        print('Usage: python run_seeds_batch.py <target> <seed1> <seed2> ...')
        sys.exit(1)
    target = sys.argv[1]
    seeds = [int(s) for s in sys.argv[2:]]
    run_batch(target, seeds)
